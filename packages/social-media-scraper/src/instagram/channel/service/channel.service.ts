...
import { CrawlInfo, ExportType, GetUserScrapeInfosDTO } from '../dto/get-user-scrape-info.dto';
...
export class ChannelService {
  ...
  async fetchUser(username: string, log: boolean = true, options: GetUserScrapeInfosDTO = { crawl: { profile: true } }): Promise<FindOneChannelDTO> {
    ...
    if (options.export) await this.handleExportOptions(username, options.export);
    if (options.download) this.downloadService.download(username, options.download);
