            ...
            modifyResponse: async ({ body }) => {
                try {
                    ...
                    if (dataObj.data["xdt_api__v1__feed__user_timeline_graphql_connection"]) {
                        ...
                        this.logger.log(`Crawl posts successfully: ${username} - ${len} - hasNextPage: ${hasNextPage}`);
                        if (!hasNextPage) {
                            scanComplete = true;
                        }
                    }
            ...
            modifyResponse: async ({ body }) => {
                try {
                    ...
                    if (dataObj && dataObj.data && dataObj.data["xdt_api__v1__clips__user__connection_v2"]) {
                        ...
                        this.logger.log(`Crawl reels successfully: ${username} - ${reelLen} - hasNextPage: ${hasNextPage}`);
                        if (!hasNextPage) {
                            scanComplete = true;
                        }
                    }
