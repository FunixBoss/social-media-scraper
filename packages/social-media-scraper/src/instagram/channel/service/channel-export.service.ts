...
export class ChannelExportService {
  ...
  async exportChannel(username: string, exportType: string | "json" | "excel"): Promise<void> {
    ...
    const folderPath = `${this.DOWNLOAD_PATH}/${username}`;
    this.createAndAccessFolder(folderPath)
    if (exportType == 'excel') this.exportChannelExcel(username, crawledTypes, folderPath);
    if (exportType == 'json') this.exportChannelJson(username, crawledTypes, folderPath);
  }

  private async exportChannelJson(username: string, crawledTypes: TCrawlingType[], folderPath: string): Promise<void> {
    ...
    const fileDir = `${folderPath}/${username}.json`
    try {
      writeFileSync(fileDir, JSON.stringify(channelFull), 'utf-8')
      this.logger.log(`Json file ${fileDir} was written successfully.`);
    } catch (error) {
      this.logger.warn(`Json file ${fileDir} was written failed.`, error);
    }
  }

  private async exportChannelExcel(username: string, crawledTypes: TCrawlingType[], folderPath: string): Promise<void> {
    const workbook = new Workbook();
    if (crawledTypes.includes("CHANNEL_PROFILE"))
    ...
    const fileDir = `${folderPath}/${username}.xlsx`
    try {
      await workbook.xlsx.writeFile(fileDir);
      this.logger.log(`Excel file ${fileDir} was written successfully.`);
    } catch (error) {
      this.logger.warn(`Excel file ${fileDir} was written failed.`, error);
    }
  }

  createAndAccessFolder(folderPath: string): void {
    try {
      mkdirSync(folderPath, { recursive: true });
      this.logger.log(`Directory ${folderPath} created.`);
    } catch (error) {
      this.logger.warn(`Directory ${folderPath} created failed.`, error);
    }
  }
}
