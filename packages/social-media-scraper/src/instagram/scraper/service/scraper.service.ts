...
export default class ScraperService {
    ...
    async test() {
        const cookies = this.convertToRightFormat(this.cookieHandler.getAsText('instagram', 'default.json'));
        const instaFetcher: InstaFetcher = InsScraperServiceFactory.create(
            cookies,
            { proxy: { protocol: "http", host: "s4.cloudvpsviet.vn", port: 8386 } }
        );
        instaFetcher.fetchHighlights("_javadeveloper")
            .then((data) => {
                writeFileSync("uploads/fetchHighlights.json", JSON.stringify(data), 'utf-8')
            })
            .catch(error => console.log("fetchHighlights", error["name"], error["message"]));
        instaFetcher.fetchPost("https://www.instagram.com/reel/CXhW_4sp32Z/")
            .then(data => {
                writeFileSync("uploads/fetchPost.json", JSON.stringify(data), 'utf-8')
            })
            .catch(error => console.log("fetchPost", error["name"], error["message"]));
        instaFetcher.fetchPostByShortcode("CXhW_4sp32Z")
            .then(data => {
                writeFileSync("uploads/fetchPostByShortcode.json", JSON.stringify(data), 'utf-8')
            })
            .catch(error => console.log("fetchPostByShortcode", error["name"], error["message"]));
        instaFetcher.fetchStories("_javadeveloper")
            .then((data) => {
                writeFileSync("uploads/fetchStories.json", JSON.stringify(data), 'utf-8')
            })
            .catch(error => console.log("fetchStories", error["name"], error["message"]));
        instaFetcher.fetchUserPosts("_javadeveloper")
            .then(data => {
                writeFileSync("uploads/fetchUserPosts.json", JSON.stringify(data), 'utf-8')
            })
            .catch(error => console.log("fetchUserPosts", error["name"], error["message"]));
        instaFetcher._getReelsIds("_javadeveloper")
            .then(data => {
                writeFileSync("uploads/_getReelsIds.json", JSON.stringify(data), 'utf-8')
            })
            .catch(error => console.log("_getReelsIds", error["name"], error["message"]));
        instaFetcher.searchFollower(await instaFetcher.getIdByUsername("_javadeveloper"), '')
            .then(data => {
                writeFileSync("uploads/searchFollower.json", JSON.stringify(data), 'utf-8')
            })
            .catch(error => console.log("searchFollower", error["name"], error["message"]));
        instaFetcher.searchFollowing(await instaFetcher.getIdByUsername("_javadeveloper"), '')
            .then(data => {
                writeFileSync("uploads/searchFollowing.json", JSON.stringify(data), 'utf-8')
            })
            .catch(error => console.log("searchFollowing", error["name"], error["message"]));
    }
